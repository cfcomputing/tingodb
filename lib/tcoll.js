    ...
    if (exists && create)
      return cb(
        new Error(
          "Collection " +
            self._name +
            " already exists. Currently in safe mode."
        )
      );
      ...
      return cb(
        new Error(
          "Collection " +
            self._name +
            " does not exist. Currently in safe mode."
        )
      );
  }
...
tcoll.prototype.initFS = function (tdb, name, options, create, cb) {
  ...
  this._tq = new wqueue(100, function (cb) {
    safe.run(
                ...
                safe.run(
                  function (cb) {
                    fs.read(
                      ...
                      45,
                      pos,
                      safe.sure(cb, function (bytes, data) {
                        if (bytes === 0) {
                          self._fsize = pos;
                          return cb();
                        }
                  ...
                  function (err) {
                    if (err)
                      cb(
                        new Error(
                          self._name + ": Error during load - " + err.toString()
                        )
                      );
                    else cb();
                  }
      ...
      function (err) {
        if (!found && err) return cb(err); // nothing read and error, just rise it
          ...
          function (cb) {
            ...
            if (deleted > size || err) {
              self._compact(function (errCompact) {
                if (errCompact && err) cb(errCompact);
                else {
...
tcoll.prototype.rename = function (nname, opts, cb) {
  ...
  if (err) return safe.back(cb, err);
  if (self._tdb._stype == "mem") {
    ...
    self._tq.add(
      function (cb) {
        fs.rename(
          path.join(self._tdb._path, self._name),
          path.join(self._tdb._path, nname),
          safe.sure(cb, function () {
            delete self._tdb._cols[self._name];
            self._tdb._cols[nname] = self;
...
tcoll.prototype.createIndex = tcoll.prototype.ensureIndex = function (
  obj,
  options,
  cb
) {
  ...
  c.sort(obj);
  if (c._err) return safe.back(cb, c._err);
  var key = c._sort;

  if (key === null) return safe.back(cb, new Error("No fields are specified"));

  var index = self._idx[key];
  if (index) return safe.back(cb, null, index.name);
...
tcoll.prototype._getM = function (pos, unsafe, cb) {
  safe.back(
    cb,
    null,
    unsafe ? this._mstore[pos - 1] : this._tdb._cloneDeep(this._mstore[pos - 1])
  );
};
...
tcoll.prototype._getFS = function (pos, unsafe, cb) {
  ...
  if (cached) return safe.back(cb, null, cached);
  var b1 = new Buffer(45);
  fs.read(
    self._fd,
    b1,
    ...
    safe.sure(cb, function (bytes, data) {
      ...
      fs.read(
        ...
        safe.sure(cb, function (bytes, data) {
          ...
          if (bytes <= self._cmaxobj) self._cache.set(pos, obj);
          cb(null, obj);
        })
      );
    })
...
tcoll.prototype.insertMany = function (docs, opts) {
  // console.log("running insertMany");
  var self = this;
  return new Promise((resolve) => {
    ...
    this._tq.add(
      ...
      true,
      (error, results) => {
        // console.log("callback z", error, results);
...
tcoll.prototype._wrapTypes = function (obj) {
  ...
  _.each(obj, function (v, k) {
    if (_.isDate(v)) obj[k] = { $wrap: "$date", v: v.valueOf(), h: v };
    ...
    else if (_.isObject(v)) self._wrapTypes(v);
  });
  return obj;
};
...
tcoll.prototype._ensureIds = function (obj) {
  ...
  _.each(obj, function (v, k) {
    if (k.length > 0) {
      if (k[0] == "$") throw new Error("key " + k + " must not start with '$'");
      if (k.indexOf(".") != -1)
...
tcoll.prototype._putM = function (item_, remove, cb) {
  ...
  self._wq.add(
    function (cb) {
      ...
      // update index
      _.each(self._idx, function (v) {
        if (!remove) v.set(item, key._id);
        else v.del(item, key._id);
      });
      cb(null);
    },
    true,
...
tcoll.prototype._putFS = function (item, remove, cb) {
  ...
  self._wq.add(
    function (cb) {
      ...
      var key = {
        _id: simplifyKey(item._id),
        _uid: self._id,
        _dt: new Date().valueOf(),
      };
      ...
      safe.run(
        function (cb) {
          ...
          fs.write(
            ...
            self._fsize,
            safe.sure(cb, function (written) {
              if (remove) delete self._store[key._id];
              else self._store[key._id] = { pos: self._fsize, sum: key._s };

              if (remove || sobj.length > self._cmaxobj)
                self._cache.unset(self._fsize);
              else self._cache.set(self._fsize, item);
              self._fsize += written;
              ...
              if (self._check1 == 0) {
                ...
                fs.fstat(
                  self._fd,
                  safe.sure(cb, function (stat) {
                    if (self._fsize != stat.size)
                      cb(
                        new Error(
                          "File size mismatch. Are you use db/collection exclusively?"
                        )
                      );
                    else cb();
                  })
...
tcoll.prototype.count = function (query, options, cb) {
  ...
  } else self.find(query, options).count(cb);
};
    ...
    if (argc > 0) {
      ...
      if (argc > 1) {
        if (argc == 2) {
          ...
          if (_(val).keys().intersection(findOpts).size() > 0) opts = val;
          else fields = val;
        } else {
          fields = arguments[1];
          if (argc == 3) opts = arguments[2];
          else {
            skip = arguments[2];
            limit = arguments[3];
          }
...
tcoll.prototype.findAsPromise = function () {
  var cb = null,
    query = {},
    opts = {},
    fields = null,
...
function simplifyKey(key) {
  var k = key;
  if (key.toJSON) k = key.toJSON();
  if (_.isNumber(k) || _.isString(k)) return k;
  return k.toString();
}
        ...
        safe.sure(cb, function (res) {
          if (res.length == 0) {
            ...
            self._get(
              ...
              safe.sure(cb, function (obj) {
                var robj =
                  opts.new && !opts.remove ? obj : self._tdb._cloneDeep(obj);
                // remove current version of doc from indexes
                _.each(self._idx, function (v) {
                  v.del(obj, simplifyKey(obj._id));
                });
...
tcoll.prototype.remove = function (query, opts, cb) {
  ...
  opts = opts || {};
  if (opts.w > 0 && !_.isFunction(cb)) throw new Error("Callback is required");
  cb = cb || function () {};
...
tcoll.prototype.__find = function (
  query,
  fields,
  skip,
  limit,
  ...
  if (!_.isEmpty(pi)) {
    ...
    if (si) {
      ...
      //	range.reverse();
      sort = null;
    } else range = _.map(self._store, "pos");
  }
  ...
  if (self._tdb._gopts.searchInArray) arrayMatch = true;
  else {
  ...
  var matcher = new Function(
    "obj",
    "return " + (arrayMatch ? qt.native3() : qt.native())
  );
  ...
  safe.eachOfSeries(
    range,
    function (pos, k, cb) {
      if (sort == null && limit && res.length >= limit) return safe.back(cb);
      self._get(
        pos,
        true,
        safe.sure(cb, function (obj) {
...
tcoll.prototype._find = function (
  query,
  fields,
  skip,
  limit,
...
tcoll.prototype.mapReduce = function (map, reduce, opts, cb) {
  ...
  if (!opts.out)
    return safe.back(cb, new Error("the out option parameter must be defined"));
  if (!opts.out.inline && !opts.out.replace) {
    return safe.back(
      cb,
      new Error("the only supported out options are inline and replace")
    );
  }
  ...
  self.find(
    ...
    safe.sure(cb, function (c) {
      var doc;
      safe.doUntil(
        function (cb) {
          c.nextObject(
            safe.sure(cb, function (_doc) {
              doc = _doc;
              if (doc) map.call(doc);
              cb();
            })
        ...
        safe.sure(cb, function () {
          ...
          var stats = {};
          if (opts.out.inline) return safe.back(cb, null, _.values(m), stats); // execute outside of trap

          // write results to collection
          safe.waterfall(
            [
              function (cb) {
                self._tdb.collection(
                  opts.out.replace,
                  { strict: 1 },
                  function (err, col) {
