...
function tcursor(tcoll, query, fields, opts) {
  ...
  _.each(fields, function (v, k) {
    ...
    if (v == 0 || v == 1) {
      ...
      if (self._fieldsType == null) self._fieldsType = v;
      if (self._fieldsType == v) {
        if (k.indexOf("_tiar.") == 0) self._arFields[k.substr(6)] = 1;
        else self._fields[k] = v;
      } else if (!self._err)
        self._err = new Error(
          "Mixed set of projection options (0,1) is not valid"
        );
    } else if (!self._err) self._err = new Error("Unsupported projection option: " + JSON.stringify(v));
  });
  // _id treated specially
  if (
    (self._fieldsType === 0 || self._fieldsType === null) &&
    self._fieldsExcludeId
  ) {
    self._fieldsType = 0;
    self._fields["_id"] = 0;
  }
...
tcursor.prototype.sort = function (v, d, cb) {
  ...
  if (self._i != 0) this._err = new Error("Cursor is closed");
  if (!this._err) {
...
tcursor.prototype.nextObject = function (cb) {
  ...
  self._ensure(
    safe.sure(cb, function () {
      if (self._i >= self._items.length) return cb(null, null);
...
tcursor.prototype.toArray = function (cb) {
  ...
  if (self.isClosed()) self._err = new Error("Cursor is closed");
  ...
  self._ensure(
    safe.sure(cb, function () {
      safe.mapSeries(
        self._i != 0
          ? self._items.slice(self._i, self._items.length)
          : self._items,
        function (pos, cb) {
          self._get(
            pos,
...
tcursor.prototype.toArrayAsPromise = function () {
  var self = this;
...
tcursor.prototype._ensure = function (cb) {
  var self = this;
  if (self._items != null) return safe.back(cb);
  self._c._find(
    self._query,
    {},
    self._skip,
    ...
    self._hint,
    self._arFields,
    safe.sure_result(cb, function (data) {
      self._items = data;
      self._i = 0;
    })
  );
...
tcursor.prototype._projectFields = function (obj) {
  ...
  if (self._fieldsType != null) {
    if (self._fieldsType == 0) {
      ...
      obj = applyProjectionGet(
        obj,
        self._fields,
        self._fieldsExcludeId ? {} : { _id: obj._id }
      );
  }
  return obj;
};
